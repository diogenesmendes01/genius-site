...
- HTML com `noindex` precisa continuar rastreável para que o buscador leia a instrução. `noindex` não substitui autenticação. A checagem do cabeçalho usa o caminho com barras normalizadas: aliases como `//matricula/` e `//dashboard/` também recebem `X-Robots-Tag: noindex`, sem reescrever a URL roteada. O bloqueio de `/api/` já existente foi preservado; seu cabeçalho funciona como instrução adicional para clientes que possam lê-lo.
...
`npm test -- --runInBand`: **21 suítes e 317 testes aprovados**, cobrindo rotas HTTP reais, metadata/canonical, referências JSON-LD, links e fragmentos, sitemap, FAQ e comportamento existente. `npm run build`: aprovado. A cobertura inclui Host com porta, aliases com barras repetidas, preservação das queries e carga semanal do schema coerente com a tabela pública. Os aliases operacionais são testados com GET/HEAD, incluindo HTML e assets; `//api/health` conserva o 404 e recebe o cabeçalho `noindex`.
